...
# root에서 선행건물들의 시간을 더해가면서 계산한다.
# 큐 형식으로 현재 건물을 건설하는데 걸리는 시간을 저장해놓고
# 그 다음 건물을 짓는데 걸리는 시간에 그 값을 더한 값을 저장한다.
# 다음 건물이 선행 건물이 여러 개인 경우 병목자원에 대한 시간을 저장해줘야한다.
# 현재 값과 들어온 값중 큰 값을 저장하는 방식으로 갱신시킨다.
# 진입차수가 없어진 노드를 큐에 넣는 식으로 계속해서 진행.
for j in range(T):
    # 건물 개수와 비교 횟수
    n, k = map(int, si().split())
    # index번째 건물 뒤에 와야할 건물들이 담길 리스트
